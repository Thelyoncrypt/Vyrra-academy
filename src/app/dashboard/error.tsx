...
/**
 * Dashboard error boundary — the error state of the dashboard's four states
 * (loading / empty / error / populated). A genuine render/runtime failure of
 * the dashboard lands here.
 *
 * DESIGN.md: the same calm editorial recovery surface the lessons route
 * established — serif display headline (Copernicus substitute, weight 400,
 * negative tracking), measured body copy, a coral primary CTA to retry and a
 * quiet text-link back to Tracks. No stack traces or raw error text in the UI
 * (security: no internal detail leakage); `digest` is the only safe
 * correlation handle, shown small and muted for support. Client component per
    ...
    <PageShell>
      <div className="mx-auto max-w-md rounded-lg border border-hairline bg-surface-card px-8 py-14 text-center">
        ...
        <h1 className="mt-4 font-display text-[2rem] font-normal leading-[1.15] tracking-[-0.5px] text-ink">
          Your dashboard didn&rsquo;t load
        </h1>
        <p className="mx-auto mt-4 max-w-sm font-sans text-[1rem] leading-[1.7] text-muted">
          Something went wrong building this view. Your progress is safe — try
          again, or jump straight into the tracks and pick up where you left
          off.
        </p>
